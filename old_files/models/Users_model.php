...
class Users_model extends Crud_model {
    ...
    function authenticate($email, $password) {
        ...
        $this->db_builder->select("id,user_type,client_id,password,region_id,office_id,division_id, email");
        $result = $this->db_builder->getWhere(array('email' => $email, 'status' => 'active', 'deleted' => 0, 'disable_login' => 0));

        $result_count = count($result->getResult()); //echo "<pre>"; print_r($result->getResult()); die;
        if (!$result_count) {
            return false;
        }
    ...
    private function verify_password($user_info, $password) {
        ...
        if ($user_info->password && (strlen($user_info->password) === 60 && password_verify($password, $user_info->password)) || $user_info->password === md5($password)) {
            ...
            if ($this->_client_can_login($user_info) !== false) {
                ...
                $session->set('region_id', $user_info->region_id);
                $session->set('office_id', $user_info->office_id);
                $session->set('division_id', $user_info->division_id);
                $session->set('user_type', $user_info->user_type);*/
                 $session->set([
                'user_id'     => $user_info->id,
                'region_id'   => $user_info->region_id,
                'office_id'   => $user_info->office_id,
                'division_id' => $user_info->division_id,
                'user_type'   => $user_info->user_type,
                'role' => $user_info->role ?? $user_info->user_type ?? 'employee',
    ...
    function get_usersdataByClientID($client_id = 0){
        $users_table = $this->db->prefixTable('users');
        $client_id = $this->_get_clean_value($client_id);
        $sql = "SELECT $users_table.* FROM $users_table   
        WHERE $users_table.deleted=0 AND $users_table.client_id=$client_id ";
    ...
    function get_access_info($user_id = 0) {
        ...
        $sql = "SELECT $users_table.id, $users_table.user_type, $users_table.is_admin, $users_table.role_id, $users_table.email,
            ...
            $roles_table.title as role_title, $roles_table.permissions,$users_table.region_id,$users_table.office_id,$users_table.division_id,
