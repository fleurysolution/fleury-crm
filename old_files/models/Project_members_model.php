...
class Project_members_model extends Crud_model {
    ...
    function get_rest_team_members_for_a_project($project_id = 0,$options=array()) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
        $project_id = $this->_get_clean_value($project_id);
        ...
        LEFT JOIN $project_members_table ON $project_members_table.user_id=$users_table.id
        WHERE $users_table.user_type='staff' AND $users_table.status='active' AND $users_table.deleted=0 AND $users_table.id NOT IN (SELECT $project_members_table.user_id FROM $project_members_table WHERE $project_members_table.project_id='$project_id' AND deleted=0)";
        if($region_id){ $sql.= " AND $users_table.region_id=$region_id"; }
        if($office_id){  $sql.= " AND $users_table.office_id=$office_id";  }
        if($division_id){  $sql.= " AND $division_id.region_id=$division_id"; }
        $sql.=" ORDER BY $users_table.first_name ASC";
        //echo $sql; die;
    }
        return $this->db->query($sql);
     ...
     function get_rest_team_members_for_a_project_respective_office_division($project_id = 0,$region_id=0, $office_id=0, $division_id=0) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
        $project_id = $this->_get_clean_value($project_id);
        $region_id = $this->_get_clean_value($region_id);
        $office_id = $this->_get_clean_value($office_id);
        $division_id = $this->_get_clean_value($division_id);

        $sql = "SELECT $users_table.id, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS member_name
        FROM $users_table LEFT JOIN $project_members_table ON $project_members_table.user_id=$users_table.id WHERE $users_table.user_type='staff' AND $users_table.status='active'  AND $users_table.deleted=0 AND $users_table.id NOT IN (SELECT $project_members_table.user_id FROM $project_members_table WHERE $project_members_table.project_id='$project_id' AND deleted=0)
        ORDER BY $users_table.first_name ASC";
        //die; 
       // echo "AND $users_table.region_id='$region_id' AND $users_table.office_id='$office_id' AND $users_table.division_id='$division_id'";
        return $this->db->query($sql);
    }


    function get_client_contacts_of_the_project_client($project_id = 0,$options=array()) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
     ...
     function get_client_contacts_of_the_project_client_in_localities($project_id = 0,$options=array()) {
        $project_members_table = $this->db->prefixTable('project_members');
        $users_table = $this->db->prefixTable('users');
        $projects_table = $this->db->prefixTable('projects');
        $project_id = $this->_get_clean_value($project_id);
        ...
        FROM $users_table
        LEFT JOIN $project_members_table ON $project_members_table.user_id=$users_table.id
        WHERE $users_table.user_type='client' AND $users_table.deleted=0 AND $users_table.client_id=(SELECT $projects_table.client_id FROM $projects_table WHERE $projects_table.id=$project_id) AND $users_table.id NOT IN (SELECT $project_members_table.user_id FROM $project_members_table WHERE $project_members_table.project_id='$project_id' AND deleted=0) AND $users_table.region_id=$projects_table.region_id AND $users_table.office_id=$projects_table.office_id AND $users_table.division_id=$projects_table.division_id;
        ORDER BY $users_table.first_name ASC";
        }else{
            $sql = "SELECT $users_table.id, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS contact_name
        FROM $users_table
