...
class Subscriptions_model extends Crud_model {
  ...
  public function get_details_subscription($client_id) {
    $subscriptions_table = $this->db->prefixTable('subscriptions');

        $sql = "SELECT * FROM $subscriptions_table WHERE client_id = ?";
        return $this->db->query($sql, [$client_id])->getRow();
    }
   public function get_details_client($subdomain) {
            $clients_table = $this->db->prefixTable('clients');
        
            $sql = "SELECT id FROM $clients_table WHERE subdomain = ?";
            $result = $this->db->query($sql, [$subdomain])->getRow();
    ...
    function update_subscription_Data($subscription_id, $data) {
