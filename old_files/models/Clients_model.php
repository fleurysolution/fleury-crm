...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
       ...
       public function get_client_id($subdomain) {
            $clients_table = $this->db->prefixTable('clients');
    ...
    function is_duplicate_subdomain_name($subdomain, $id = 0) {

        $result = $this->get_all_where(array("subdomain" => $subdomain, "is_lead" => 0, "deleted" => 0));
        if (count($result->getResult()) && $result->getRow()->id != $id) {
            return $result->getRow();
        } else {
            return false;
