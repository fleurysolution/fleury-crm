...
class Contracts_model extends Crud_model {
    ...
    public function get_parent_contract_id($contract_id){
        $contracts_table = $this->db->prefixTable('contracts');
        $sql="SELECT parent_contract_id FROM $contracts_table WHERE id = $contract_id ORDER BY id DESC";
        $result= $this->db->query($sql)->getResult();
        return $result->parent_contract_id;

    }
   /* public function getLatestContractVersion($contract_id) {
        
        $contracts_table = $this->db->prefixTable('contracts');
        $sql="SELECT * FROM $contracts_table WHERE id = $contract_id ORDER BY id DESC";
        $result= $this->db->query($sql)->getRow();
        //$result = $this->where('parent_contract_id', $contract_id)->orderBy('id', 'DESC') ->findAll(); 
    ...
    public function getLatestContractVersion($contract_id,$parent_contract_id) {
    // Define the contracts table with prefix
    $contracts_table = $this->db->prefixTable('contracts');
    if($parent_contract_id ==''){
        $sql = "SELECT * FROM $contracts_table WHERE parent_contract_id = $contract_id ORDER BY version_number DESC, id DESC LIMIT 1";
    }else{
        $sql = "SELECT * FROM $contracts_table WHERE parent_contract_id = $parent_contract_id ORDER BY version_number DESC, id DESC LIMIT 1";
    }
    

    // Execute query and get the latest row
    $result = $this->db->query($sql)->getRow();

    // Return result if found, otherwise return null
    return !empty($result) ? $result : null;
}
/*
    public function getLatestContractVersion($contract_id) {
    $contracts_table = $this->db->prefixTable('contracts');
    $sql = "SELECT * FROM $contracts_table WHERE parent_contract_id = $contract_id ORDER BY id DESC LIMIT 1";
    $result = $this->db->query($sql)->getRow();
    ...
    {
        // Insert the new version data into the table

        $tableName = $this->db->prefixTable('contracts_versions');
    ...
    public function getLastVersionNumber($contractId)
    {
     ...
     public function getAllContractVersions($contract_id)
    {   
        ...
        $sql_version="SELECT * FROM $contracts_table WHERE parent_contract_id = $parent_contract_id ORDER BY version_number DESC";
        $result_versions= $this->db->query($sql_version);
