...
class Invoices_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $invoices_table.*, $clients_table.currency, $clients_table.currency_symbol, $clients_table.company_name, $projects_table.title AS project_title, $projects_table.price AS project_price, credit_note_table.id AS credit_note_id, credit_note_table.display_id AS credit_note_display_id, main_invoice_table.display_id AS main_invoice_display_id, recurring_invoice_table.display_id AS recurring_invoice_display_id,
    ...
    function get_details_with_project_price($options = array()) {
        $invoices_table = $this->db->prefixTable('invoices');
        $clients_table = $this->db->prefixTable('clients');
        $projects_table = $this->db->prefixTable('projects');
        $taxes_table = $this->db->prefixTable('taxes');
     ...
     function get_active_invoice_details($options = array()) {
        $invoices_table = $this->db->prefixTable('invoices');
        $clients_table = $this->db->prefixTable('clients');
        $projects_table = $this->db->prefixTable('projects');
        $taxes_table = $this->db->prefixTable('taxes');
        ...
        LEFT JOIN (SELECT $taxes_table.* FROM $taxes_table) AS tax_table2 ON tax_table2.id = $invoices_table.tax_id2
        LEFT JOIN (SELECT $taxes_table.* FROM $taxes_table) AS tax_table3 ON tax_table3.id = $invoices_table.tax_id3
        LEFT JOIN (SELECT invoice_id, SUM(amount) AS payment_received FROM $invoice_payments_table WHERE deleted=0 AND status=1 GROUP BY invoice_id) AS payments_table ON payments_table.invoice_id = $invoices_table.id 
        LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $invoices_table.id 
        $join_custom_fieds
        WHERE $invoices_table.deleted=0 AND $invoices_table.status NOT IN ('cancelled','void') $where $custom_fields_where";
        return $this->db->query($sql);
    }

    function get_cancelled_invoice_details($options = array()) {
        $invoices_table = $this->db->prefixTable('invoices');
        $clients_table = $this->db->prefixTable('clients');
        $projects_table = $this->db->prefixTable('projects');
        $taxes_table = $this->db->prefixTable('taxes');
        ...
        LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $invoices_table.id 
        $join_custom_fieds
        WHERE ($invoices_table.deleted=1 OR $invoices_table.status IN ('cancelled', 'void')) $where $custom_fields_where";
        // echo $sql; die;
        return $this->db->query($sql);
    }
    ...
    function get_invoice_total_summary($invoice_id) {
        ...
        $project_sql = "SELECT $projects_table.price AS project_price FROM $projects_table WHERE $projects_table.id=(SELECT $invoices_table.project_id FROM $invoices_table WHERE $invoices_table.id=$invoice_id LIMIT 1)";
        $projects = $this->db->query($project_sql)->getRow();
        //print_r($projects->project_price); die;
        if(!empty($projects)){
        $result->project_price=$projects->project_price;
            }else{

                $result->project_price=0;
            }
        ...
        WHERE $invoice_payments_table.deleted=0 AND $invoice_payments_table.status=1  AND $invoice_payments_table.invoice_id=$invoice_id";
        ...
        $payment_sql_pending_balance = "SELECT SUM($invoice_payments_table.amount) AS total_pending_confirmation_balance
        FROM $invoice_payments_table
        WHERE $invoice_payments_table.deleted=0 AND $invoice_payments_table.status=0  AND $invoice_payments_table.invoice_id=$invoice_id";

        $payment_pending_balance = $this->db->query($payment_sql_pending_balance)->getRow();
        $result->total_pending_confirmation_balance=$payment_pending_balance->total_pending_confirmation_balance;
    ...
    function update_payment_status($payment_id = 0, $data=[]) {
       /* $status = $this->_get_clean_value(array("status" => $status), "status");
        $status_data = array("status" => $status);*/
        return $this->ci_save($data, $payment_id);
    }
