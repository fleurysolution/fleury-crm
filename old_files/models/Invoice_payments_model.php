...
class Invoice_payments_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $invoice_payments_table.*, $invoices_table.client_id, $invoices_table.display_id, (SELECT $clients_table.currency_symbol FROM $clients_table WHERE $invoice_payments_table.status=1 AND $clients_table.id=$invoices_table.client_id limit 1) AS currency_symbol, $payment_methods_table.title AS payment_method_title, $projects_table.title AS project_title, $projects_table.price AS project_price
    ...
    function get_approved_details($options = array()) {
        $invoice_payments_table = $this->db->prefixTable('invoice_payments');
        $invoices_table = $this->db->prefixTable('invoices');
        $payment_methods_table = $this->db->prefixTable('payment_methods');
        $clients_table = $this->db->prefixTable('clients');
        ...
        LEFT JOIN $projects_table ON $projects_table.id= $invoices_table.project_id
        LEFT JOIN $payment_methods_table ON $payment_methods_table.id = $invoice_payments_table.payment_method_id
        WHERE $invoice_payments_table.deleted=0 AND $invoice_payments_table.status=1 AND $invoices_table.deleted=0 $where";
        return $this->db->query($sql);
    }

    function get_pending_details($options = array()) {
        $invoice_payments_table = $this->db->prefixTable('invoice_payments');
        $invoices_table = $this->db->prefixTable('invoices');
        $payment_methods_table = $this->db->prefixTable('payment_methods');
        $clients_table = $this->db->prefixTable('clients');
        ...
        LEFT JOIN $payment_methods_table ON $payment_methods_table.id = $invoice_payments_table.payment_method_id
        LEFT JOIN $projects_table ON $projects_table.id= $invoices_table.project_id
        WHERE $invoice_payments_table.deleted=0 AND $invoice_payments_table.status=0 AND $invoices_table.deleted=0 $where";
        return $this->db->query($sql);
    }
    ...
    public function getUserByID($userid){
        $tableBuilder = $this->db->prefixTable('users');
        $sql="select * from $tableBuilder where id=$userid or client_id=$userid";
