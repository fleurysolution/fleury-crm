...
class Subscriptions extends Security_Controller
{
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        if ($data->no_of_cycles_completed > 0 && $data->no_of_cycles_completed == $data->no_of_cycles) {
         ...
         if ($data->max_no_of_staff) {
             $max_no_of_staff=$data->max_no_of_staff;
            }
        if ($data->max_no_of_client) {
             $max_no_of_client=$data->max_no_of_client;
            }
        if ($data->invoice_limit) {
             $invoice_limit=$data->invoice_limit;
            }
        if ($data->project_limit) {
              $project_limit=$data->project_limit;
            }
        if ($data->storage_limit) {
            $storage_limit= $data->storage_limit;
            }
        if ($data->enhanced_reporting) {
             $enhanced_reporting=$data->enhanced_reporting;
            }
        if(empty($data->company_name)){
            $data->company_name='View';
        }
