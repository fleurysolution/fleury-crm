...
class Contract extends Security_Controller {
    ...
    function accept_contract() {
        ...
        $contract_info = $this->Contracts_model->get_one($contract_id);
        $contract_items=$this->Contract_items_model->get_details(array('contract_id'=>$contract_id))->getResult();
        $discount=$contract_info->discount_amount;
        $discount_amount_type=$contract_info->discount_amount_type;
        $itemsum=0;
        foreach($contract_items as $contract_item){
            $itemsum =$itemsum+$contract_item->total;
        }
        if($discount_amount_type=='fixed_amount'){
            $total_amount=$itemsum-$discount;
        }else{
            $total_amount=$itemsum-$itemsum*$discount/100;
        }
        $amended_price=$total_amount;

        $reason= 'Amendment for : <a href="https://bpms247.com/contracts/view/'.$contract_id.'"> Contract #'.$contract_id.'</a>';
        ...
        if ($this->Contracts_model->ci_save($contract_data, $contract_id)) {
            // Project Price Ammendments Starts here
             if ($project_id) {
                    $data = array(
