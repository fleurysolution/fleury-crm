...
class Team_members extends Security_Controller {
    ...
    function __construct() {
        ...
        $this->Availability_model=model('Availability_model');
    }
    ...
    function appointments_info($user_id) {
        validate_numeric_value($user_id);
        $this->can_access_team_members_note($user_id);
        if ($user_id) {

           $view_data['team_availability'] = $this->Availability_model->get_team_member_availability($user_id);
           // print_r($view_data); die;
            $view_data['user_id'] = clean_data($user_id);
            return $this->template->view("team_members/appointments_setting/index", $view_data);
        }
    }


    function availability_list_data($id = 0) {
        // $this->can_access_notes();
        validate_numeric_value($id);
        $list_data = $this->Availability_model->get_team_member_availability($id)->getResult();
        $result = array(); //print_r($list_data); die;
        foreach ($list_data as $data){
            $result[] = $this->_make_availability_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    private function _make_availability_row($data) {
        $public_icon = "";
        // if ($data->is_public) {
