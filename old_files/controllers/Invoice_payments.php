...
class Invoice_payments extends Security_Controller {
    ...
    function pending_payments() {
        if ($this->login_user->user_type === "staff") {
            ...
            $view_data['client_id'] = $this->login_user->client_id;
            $view_data['page_type'] = "full";
            return $this->template->rander("clients/payments/pending-payments", $view_data);
        }
    }
    ...
    function confirm_payment_modal_form() {
        //$this->access_only_allowed_members();
        ...
        return $this->template->view('invoices/confirm_payment_modal_form', $view_data);
    }
    ...
    function save_payment() {
       ...
       //echo $this->login_user->id; die;
        ...
        if ($invoice_payment_id) {
            ...
            return json_encode(array("success" => true, "invoice_id" => $item_info->invoice_id, "data" => $this->_make_payment_row($item_info), "invoice_total_view" => $this->_get_invoice_total_view($item_info->invoice_id), 'id' => $invoice_payment_id, 'message' => app_lang('record_saved')));
        } else {
    ...
    function save_payment_status() {
        ...
        $invoice_payment_data = array(
            ...
            "status" => '1',
            "approval_note"=>$this->request->getPost('invoice_payment_confirmation_note'),
        );
        ...
        if ($invoice_payment_id) {
           ...
           //send welcome email
                $email_template = $this->Email_templates_model->get_final_template("invoice_payment_confirmation"); //use default template since creating new client

                $parser_data["SIGNATURE"] = $email_template->signature;
                $parser_data["INVOICE_ID"] = $invoice_id;
                $payment_amount=floatval(str_replace(",", "", $this->request->getPost('invoice_payment_amount')));
                $parser_data["PAYMENT_AMOUNT"] = to_currency($payment_amount);
                ...
                $options = array("client_id" => $item_info->client_id);
                $contact = $this->Users_model->get_usersdataByClientID($item_info->client_id);
                send_app_mail($contact->email, $subject, $message);
            //get payment data
            //$options = array("id" => $invoice_payment_id);
            //$item_info = $this->Invoice_payments_model->get_details($options)->getRow(); 
    ...
    function approved_payment_list_data($invoice_id = 0) {
        if (!$this->can_view_invoices()) {
            app_redirect("forbidden");
        }
        ...
        $list_data = $this->Invoice_payments_model->get_approved_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_payment_row($data);
        }
    ...
    function pending_payment_list_data($invoice_id = 0) {
        if (!$this->can_view_invoices()) {
            app_redirect("forbidden");
        }
        ...
        $list_data = $this->Invoice_payments_model->get_pending_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_payment_row($data);
        }
    ...
    function approved_payment_list_data_of_client($client_id = 0) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
    ...
    function pending_payment_list_data_of_client($client_id = 0) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_payment_row($data) {
        ...
        if($data->status=='1'){ $payment_status='<button class="btn btn-success">Paid</button>'; }else{ 
                if($this->login_user->user_type == "staff"){
                $payment_status='<button class="btn btn-warning">Pending Confirmation</button>';
                $payment_status.= modal_anchor(get_uri("invoice_payments/confirm_payment_modal_form"), "<i data-feather='check' class='icon-16 '></i>", array("class" => "edit", "title" => app_lang('edit_payment'), "data-post-id" => $data->id, "class"=>"btn btn-success", "data-post-invoice_id" => $data->invoice_id,));
                }else{
                    $payment_status='<button class="btn btn-warning">Pending Confirmation</button>';
                }
             }
             // $options=array('id'=>$data->created_by);
             //echo $data->created_by; die;
        $usersData=$this->Invoice_payments_model->getUserByID($data->created_by);
        //print_r($usersData); 
        $username=$usersData[0]->first_name.' '.$usersData[0]->last_name;
    ...
    public function approve_payment($payment_id){
        //echo $payment_id;
        $data['status']='1';
        $this->Invoice_payments_model->update_payment_status($payment_id, $data);
        app_redirect('invoice_payments');
