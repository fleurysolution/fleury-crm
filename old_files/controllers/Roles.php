...
class Roles extends Security_Controller
{
    ...
    function permissions($role_id)
    {
        if ($role_id) {
            ...
            $view_data['can_assign_asset_on_projects'] = get_array_value($permissions, "can_assign_asset_on_projects");
    ...
    function save()
    {
        ...
        $data = array(
            ...
            "parent_id" => $this->request->getPost('parent_role'),
            "description" => $this->request->getPost('role_description'),            
        ...
        if ($save_id) {
            $role_hyrarchy_data=array(
                'role_id'=>$save_id,
                'reports_to_role_id'=>$this->request->getPost('parent_role'),
                'hierarchy_level'=>$this->request->getPost('hierarchy_level'),            
            );
            $hierarchy_level_id = $this->Roles_model->insert_hierarchy_level($role_hyrarchy_data, $id);
    ...
    function save_permissions()
    {
        ...
        $can_assign_asset_on_projects = $this->request->getPost('can_assign_asset_on_projects');
        ...
        $permissions = array(
            ...
            "can_assign_asset_on_projects"=>$can_assign_asset_on_projects,
        );
    ...
    function save_user_role()
    {
      ...
      /**
     * View role hierarchy (returns nested array / tree)
     */
    public function get_hierarchy()
    {
        $roles = $this->Roles_model->get_details()->getResultArray();
        $tree = $this->build_hierarchy_tree($roles);
        return $this->response->setJSON($tree);
    }
    /**
     * Fetch all sub-roles (downline) recursively using CTE
     */
    public function get_sub_roles($role_id)
    {
        $data = $this->Roles_model->get_sub_roles($role_id);
        return $this->response->setJSON(["success" => true, "data" => $data]);
    }

    /**
     * Fetch all parent roles (chain of reporting)
     */
    public function get_parent_roles($role_id)
    {
        $data = $this->Roles_model->get_parent_roles($role_id);
        return $this->response->setJSON(["success" => true, "data" => $data]);
    }

    /**
     * Helper: Build tree for adjacency list fallback / view
     */
    private function build_hierarchy_tree($roles, $parent_id = null)
    {
        $branch = [];
        foreach ($roles as $role) {
            if ($role['parent_id'] == $parent_id) {
                $children = $this->build_hierarchy_tree($roles, $role['id']);
                if ($children) {
     ...
     // Show hierarchy tree
    public function hierarchy_tree()
    {
        $data['tree'] = $this->Roles_model->get_role_hierarchy_tree();
        //return view('roles/hierarchy_tree', $data);
         return $this->template->view('roles/hierarchy_tree', $data);
    }
}
