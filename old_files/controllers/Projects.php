...
class Projects extends Security_Controller {
    ...
    public function __construct() {
        ...
        // Project assets assignment 
         $this->projectsModel     = model('App\Models\Projects_model'); // your existing model
        $this->assetsModel       = model('plugins\Assets_Management\Models\Assets_model');   // wrapper for pcm_assets
        $this->assetActionsModel = model('plugins\Assets_Management\Models\Asset_actions_model'); // wrapper for pcm_asset_actions
        //$this->assignmentModel   = model('plugins\Assets_Management\Models\Pcm_project_asset_assignments_model'); // created earlier
         $this->Asset_billing_type_model=model('plugins\Assets_Management\Models\Asset_billing_type_model');
        $this->unitsTable        = db_connect()->table('pcm_asset_units');
    ...
    private function can_delete_projects($project_id = 0) {
	 ...
	 protected function validate_access_to_materials() {
...
/*add amendments*/
    ...
    function modal_form() {
        ...
        $view_data["estimate_title"] = $this->request->getPost('estimate_title');
        $view_data["estimate_scope"] = base64_decode($this->request->getPost('estimate_scope'));
        $view_data["estimate_total"] = base64_decode($this->request->getPost('estimate_total'));
        $view_data["estimate_location"] = base64_decode($this->request->getPost('estimate_location'));
        ...
        $view_data["can_edit_projects"] = $this->can_edit_projects();        
        $view_data['regions'] = $this->Settings_model->get_regions();
    ...
    function save() {
        ...
        if ($save_id) {
            ...
            if (!$id) {
                ...
                //created from estimate? save the project idcontext_id
                if ($context == "estimate") {
                    ...
                    $contract_data = array(
                        "client_id" => ($project_type === "internal_project") ? 0 : $this->request->getPost('client_id'),
                        "title" => $this->request->getPost('title'),
                        "project_id" => $save_id ? $save_id : 0,
                        "contract_date" => $this->request->getPost('start_date'),
                        "valid_until" => $this->request->getPost('deadline'),
                        "tax_id" =>  0,
                        "tax_id2" =>  0,
                        "company_id" => ($project_type === "internal_project") ? 0 : $this->request->getPost('client_id'),
                        "note" => '',
                    ...
                    $this->storeContractVersion($contract_id, $contract_data,'');
                    $this->_copy_related_items_to_contract($context_id, $contract_id);

                    $this->Projects_model->updateProjectTask($save_id,$context_id);
                ...
                $amendment_data=array(
                    'project_id'=>$save_id,
                    'amended_price'=>unformat_currency($this->request->getPost('price')),
                ...
                $this->Project_amendment_model->save_amendment($amendment_data);
    ...
    function view($project_id = 0, $tab = "", $folder_id = 0) {
        ...
        $view_data["show_assets_info"] =  true;
        $view_data["show_return_assets_info"] =  true;
    ...
    // Print Sagregated Invoice for project

    //view html is accessable to client only.
    function project_invoice_preview($invoice_id = 0, $show_close_preview = false) {
        if ($invoice_id) {
            validate_numeric_value($invoice_id);
            
            $view_data = get_project_invoice_making_data($invoice_id,$this->login_user->client_id);
           
            //$this->_check_invoice_access_permission($view_data);

            $view_data['invoice_preview'] = prepare_project_invoice_pdf($view_data, "html");
            //die;
            //show a back button
           // $view_data['show_close_preview'] = $show_close_preview && $this->login_user->user_type === "staff" ? true : false;
            ...
            return $this->template->rander("invoices/project_invoice_preview", $view_data);
        } else {
            show_404();
        }
    }
   ...
   private function _get_invoice_status_label($data, $return_html = true) {
    return get_project_invoice_status_label($data, $return_html);
}
    ...
    function project_member_modal_form() {
       ...
       // $view_data['model_info'] = $this->Project_members_model->get_one_where($where);
        $view_data['project_info']=$this->Projects_model->get_one($this->request->getPost('project_id'));
        $project_id = $this->request->getPost('project_id') ? $this->request->getPost('project_id') : $view_data['model_info']->project_id;
        $region_id = $this->request->getPost('region_id');
        $office_id = $this->request->getPost('office_id');
        $division_id = $this->request->getPost('division_id');
        $view_data['region_id'] = $region_id;
        $view_data['office_id'] = $office_id;
        $view_data['division_id'] = $division_id;
        $this->init_project_permission_checker($project_id);
        ...
        if ($add_user_type == "client_contacts") {
            ...
            $options['office_id'] = $office_id;
            $options['division_id'] = $division_id;
            $contacts = $this->Project_members_model->get_client_contacts_of_the_project_client($project_id,$options)->getResult();
            ...
            $options['office_id'] = $office_id;
            $options['division_id'] = $division_id;
             $users = $this->Project_members_model->get_rest_team_members_for_a_project($project_id,$options)->getResult(); //print_r($users);    
    ...
    // Ammendments  projects
    /* load project amendments add/edit modal */

    function project_amendments_modal_form() {
        $view_data['model_info'] = $this->Project_members_model->get_one($this->request->getPost('id'));
        $project_id = $this->request->getPost('project_id') ? $this->request->getPost('project_id') : $view_data['model_info']->project_id;
        $this->init_project_permission_checker($project_id);
        ...
        return $this->template->view('projects/project_amendment/modal_form', $view_data);
    }

    /* add a project members  */

    function save_project_amendment() {
        $project_id = $this->request->getPost('project_id');
        
        $amended_price=$this->request->getPost('amended_price');
        ...
        $already_exists = false;
                ...
                if ($project_id) {
                    $data = array(
                        "project_id" => $project_id,
                        "amended_price" => $amended_price,
                        "reason" => $reason,
                        "amendment_date" => date('m-d-Y h:i a'),
                        "user_id" => $_SESSION['user_id']
                    );

                    $save_id = $this->Project_amendment_model->save_amendment($data);
                    if ($save_id && $save_id != "exists") {
                        $save_ids[] = $save_id;
                        $final_price_update=$this->Project_amendment_model->project_amended_price($project_id);
                    } else if ($save_id === "exists") {
                        $already_exists = true;
                    }
        ...
        if (count($save_ids)) {         
            $projectData=$this->Projects_model->get_details(array('id'=>$project_id))->getResult(); 
             $client_id=$projectData[0]->client_id;
            //print_r($projectData);
            $message="Hey, <br><br>
Your project has been updated, Please check the details by login to ".base_url().'.

<br><br>
Thank you <br>
Admin,<br>
Fleury Solutions LLC';
            $this->mailalert($projectData,$client_id,$message);
            echo json_encode(array("success" => true, "data" => $save_id, 'id' => $save_id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }

    /* delete/undo a project amendments  */

    function delete_project_amendment() {
        $id = $this->request->getPost('id');
        $project_member_info = $this->Project_amendment_model->get_one($id);
        $project_id=$project_member_info->project_id;
        $this->init_project_permission_checker($project_member_info->project_id);
        if (!$this->can_add_remove_project_members()) {
            app_redirect("forbidden");
        }
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Project_amendment_model->delete($id, true)) {
                $final_price_update=$this->Project_amendment_model->project_amended_price($project_id);
                echo json_encode(array("success" => true, "data" => $this->_project_amendments_row_data($id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Project_amendment_model->delete($id)) {

                $project_amendment_info = $this->Project_amendment_model->get_one($id);
                $final_price_update=$this->Project_amendment_model->project_amended_price($project_id);
                log_notification("project_amendment_deleted", array("project_id" => $project_amendment_info->project_id, "to_user_id" => $project_amendment_info->user_id));
                echo json_encode(array("success" => true, 'message' => app_lang('record_deleted')));
            } else {
                echo json_encode(array("success" => false, 'message' => app_lang('record_cannot_be_deleted')));
            }
    ...
    /* list of project amendments, prepared for datatable  */

    function project_amendment_list_data($project_id = 0, $user_type = "") {
        validate_numeric_value($project_id);
        $this->access_only_team_members();
        $this->init_project_permission_checker($project_id);

        $options = array("project_id" => $project_id);
        $list_data = $this->Project_amendment_model->get_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_project_amendments_row($data, false);
        }
        echo json_encode(array("data" => $result));
    }

    /* return a row of project amendments list */

    private function _project_amendments_row_data($id) {
        $options = array("id" => $id);
        $data = $this->Project_amendment_model->get_details($options)->getRow();
        return $this->_make_project_member_row($data);
    }

    /* prepare a row of project member list */

    private function _make_project_amendments_row($data, $can_send_message_to_client = false) {
        //print_r($data); die;
        $link='';
        if ($this->can_add_remove_project_members()) {
            $delete_link = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_amendment'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_project_amendment"), "data-action" => "delete"));
        ...
        if($data->amended_price==''){
            $data->amended_price=0;
        }
        $id=$data->id;
        $reason=$data->reason;
        $amended_price=to_currency($data->amended_price);
        $created_at=$data->created_at;
        $newDate = date("m-d-Y", strtotime($created_at));
        
      
        return array($id, $reason,$amended_price,$newDate, $link);
    }
    ...
    function milestones_list_data($project_id = 0) {
     ...
     function milestones_list_data_for_model($project_id = 0) {
        validate_numeric_value($project_id);
        $this->init_project_permission_checker($project_id);
        ...
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_milestone_row_for_model($data);
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_milestone_row($data) {
     ...
     private function _make_milestone_row_for_model($data) {
    ...
    // Share email and print 
     public function shareProjectstoteam(){
        //echo "I am here"; print_r($this->request->getPost()); die;
        if(!empty($this->request->getPost('checkbox_file'))){
               ...
               if($this->request->getPost('submit')=='Share Now'){ 
                    return $this->template->rander("projects/files/share_file_form", $view_data);
                }else{
                    return view("projects/files/print_file_report", $view_data);
                }
    ...
    // public function 
        public function sendProjectsMailnow(){
             if(!empty($this->request->getPost('checkbox_file'))){  
                ...
                for($i=0;$i < count($checkbox_file);$i++){
                   echo $projects=$this->request->getPost('project_ids_data')[$i];
                   echo "<br> :::". $clients=$this->request->getPost('client_ids_data')[$i];
                   //  If the flie data type is projects
                        if($projects != '0'){
                            $options = array(
                                "id" => $checkbox_file[$i],
                                "project_id"=>$projects           
                           ...
                           echo $file_name = $file_info[0]->file_name;
                           echo $attachment_file=base_url().'/files/project_files/'.$projects.'/'.$file_name;   
                            echo " Hi I am the project ";                 
                        }

                        // If the data type is client data
                         if($clients != '0'){
                            $options = array(
                                "id" => $checkbox_file[$i],
                                "client_id"=>$clients            
                        ...
                        // client & project type data not available
...
function capture_image(){
        ...
        return $this->template->view('projects/files/capture-image', $view_data);
    }
    ...
    public function save_captured_image()
{
    $project_id = $this->request->getPost('project_id');
    $category_id = $this->request->getPost('category_id');    
    $description = $this->request->getPost('description');
    ...
    if ($imageData) {
        ...
        // Define main storage path (same as Rise file uploads)
        $target_path = get_setting("project_file_path");
        if (!$target_path) {
            $target_path = "files/project_files"; // fallback
        }

        // Full project folder path
        $project_folder = rtrim($target_path, "/") . "/" . $project_id . "/";
        ...
        if (file_put_contents($file_path, $decoded_image) === false) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Failed to write captured image file.'
            ]);
        }
        ...
        $file_info = [
            'file_name' => $file_name,
            'file_id' => make_random_string(16),
            'service_type' => 'camera_upload'
        ];
        ...
        if ($success) {
            save_custom_fields("project_files", $success, $this->login_user->is_admin, $this->login_user->user_type);
            log_notification("project_file_added", [
                "project_id" => $project_id,
                "project_file_id" => $success
            ]);
 ...
 /* Share Now multiple files as zip */

    function share_now_multiple_files($files_ids = "") {

        if ($files_ids) {
             $files_ids_array = explode('-', $files_ids);
             $files = $this->Project_files_model->get_files($files_ids_array);
             $projectData=$this->_get_project_info_data($files->getResult()[0]->project_id);
             $project_id=$files->getResult()[0]->project_id;
              $clientid=$projectData['project_info']->client_id;
              $this->sendMailnow($files_ids,$clientid,$project_id,'list');
        }
    }

    public function mailalert($projectData='',$client_id='',$messages=''){
                $ccmail='admin@fleurysolutions.com';   
                if(!empty($client_id) ){
                        $responseUser=$this->Projects_model->get_client_primary_contact($client_id)->getResult();
                        $emailsto=$responseUser[0]->email; 
                        $mailresponse=send_app_mail($emailsto, "The project has been updated- Fleury Solutions LLC",$messages);
                        $mailresponse=send_app_mail($ccmail, "The project has been updated- Fleury Solutions LLC",$messages);                     
                }              
    }

    // public function 
        public function sendMailnow($file,$clientid,$project_id,$user_type='list'){
             if(!empty($file)){  
                $ccmail='admin@fleurysolutions.com';
                $checkbox_file=explode('-', $file);;
                for($i=0;$i < count($checkbox_file);$i++){
                    ...
                    $file_name = $file_info[0]->file_name;
                    $attachment_file=base_url().'/files/project_files/'.$project_id.'/'.$file_name;  
                    $file_paths['file_path'][]= $attachment_file;
                    $email_options["attachments"][] = array("file_path" => $attachment_file);
                    } 
...
Fleury Solutions';
                ...
                if($user_type=='list' && !empty($clientid) ){
                   $responseUser=$this->Projects_model->get_client_primary_contact($clientid)->getResult();
                        $emailsto=$responseUser[0]->email; 
                        $mailresponse=send_app_mail($emailsto, "File from Fleury Solutions PCM",$messages ,$email_options);
                        $mailresponse=send_app_mail($ccmail, "File from Fleury Solutions PCM",$messages ,$email_options);
                     
                }
                if($mailresponse){ 
                    echo json_encode(array("success" => true, 'message' => app_lang('file_shared')));
                }else {
                    echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
    ...
    public function emailform($maildata){
        print_r($maildata);
        redirect('https://google.com');
    }
    public function sharetoteam(){
        if(!empty($this->request->getGet('checkbox_file'))){
               $view_data['checkbox_file'] = $this->request->getGet('checkbox_file');
               $view_data["users"] = $this->Messages_model->get_users_for_messaging($this->get_user_options_for_query('staff'))->getResult();
               $view_data["clients"] = $this->Messages_model->get_users_for_messaging($this->get_user_options_for_query('client'))->getResult();
    ...
    private function _make_file_row($data, $custom_fields) {
        ...
        $description = "<div class='float-start text-wrap text-break-space'>" .
            js_anchor($data->file_name, array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "1", "data-group" => "project-files", "data-url" => get_uri("projects/view_file/" . $data->id)));
        ...
        if ($data->description) {
            ...
            $file_path = get_uri("files/project_files/" . $data->project_id . '/' . $data->file_name);
       ...
       $checkmark =  '<input type="hidden" name="client_id[]" value="0">
       <input type="hidden" name="project_id[]" value="'.$data->project_id.'">
       <input type="checkbox" name="checkbox_file[]" value="'.$data->id.'">';
        get_uri("files/project_files/".$data->project_id.'/' . $data->file_name);

        $row_data = array(
            $checkmark .$data->id,
            $file_display  . $description,
 ...
 function pending_payments($project_id) {
        validate_numeric_value($project_id);
        $this->access_only_team_members();
        if ($project_id) {
            $view_data['project_info'] = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
    ...
    /* Load materials list*/
    function areas($project_id,$client_id=0){
		
         //$this->access_only_team_members_or_client_contact($client_id);
        validate_numeric_value($project_id);
        $view_data['categories_dropdown'] = $this->_get_categories_dropdown();
        $view_data['area_dropdown'] = $this->_get_area_dropdown();
        //echo $project_id;
        if ($project_id) {
            $view_data['project_id'] = $project_id;
            $view_data['project_info'] = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
            $view_data['area_data'] = $this->Material_area_model->get_details(array("project_id" => $project_id))->getResult();
            $view_data['category_data'] = $this->Material_categories_model->get_details(array("project_id" => $project_id, "deleted"=>0))->getResult();
        
            $view_data["can_edit_invoices"] = $this->can_edit_invoices();
            return $this->template->view("projects/materials/materials", $view_data);
          }
    }

    //get data for items category list

     function area_modal_form($project_id='') {
        $this->validate_submitted_data(array(
            "id" => "numeric"
        ));
...
//        $view_data['categories_dropdown'] = $this->Material_categories_model->get_dropdown_list(array("title"));
        ...
        return $this->template->view('projects/materials/area_model_form', $view_data);
    }
	
	 function materials_modal_form($project_id='') {
		 
		 
        $this->access_only_team_members();
        $this->validate_access_to_materials();

        $this->validate_submitted_data(array(
            "id" => "numeric"
        ));

        $view_data['model_info'] = $this->Materials_model->get_one($this->request->getPost('id'));
        $view_data['categories_dropdown'] = $this->Material_categories_model->get_dropdown_list(array("title"));
        $view_data['area_dropdown'] = $this->Material_area_model->get_dropdown_list(array("areaname"));
		$view_data['project_id']=$project_id;
		
		

        return $this->template->view('projects/materials/add_materials', $view_data);
    }
	...
	function material_save() {
        ...
        if ($id) {
            $item_info = $this->Materials_model->get_one($id);
        ...
        $item_id = $this->Materials_model->ci_save($item_data, $id);
        if ($item_id) {
            $options = array("id" => $item_id);
            $item_info = $this->Materials_model->get_details($options)->getRow();
        ...
        return array(
            modal_anchor(get_uri("materials/view"), $show_in_client_portal_icon . $data->title, array("title" => app_lang("item_details"), "data-post-id" => $data->id)),
            custom_nl2br($data->description ? $data->description : ""),
            $data->category_title ? $data->category_title : "-",
            $type,
            to_decimal_format($data->rate),
            modal_anchor(get_uri("materials/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id))
    ...
    function area_list_data($project_id) {
        $list_data = $this->Material_area_model->get_details(array("project_id" => $project_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _area_make_row($data) {
        $url=modal_anchor(get_uri("projects/categories_data/$data->project_id/$data->id"), "View Categories", array("class" => "edit", "title" => app_lang('view_categories'), "data-post-id" => $data->id));

        //'<a href="'.get_uri("projects/categories_data/$data->project_id/$data->id").'"> View categories </a>';
        return array($data->areaname,
            substr($data->description, 0, 80),
            $url,
            modal_anchor(get_uri("projects/area_modal_form/".$data->project_id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_items_area'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_items_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/area_delete"), "data-action" => "delete"))
        );
    }
    ...
    function area_delete() {
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Material_area_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_area_row_data($id), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
    ...
    // Area Categories list
    function category_modal_form($project_id='') {
        $this->validate_submitted_data(array(
            "id" => "numeric"
        ));
        $view_data['area_id'] = '';
		$view_data['area_dropdown'] = $this->Material_area_model->get_details(array("project_id" => $project_id))->getResult(); 
            
        $view_data['categories_dropdown'] = $this->Material_categories_model->get_dropdown_list(array("title"));
        
        $view_data['model_info'] = $this->Material_categories_model->get_one($this->request->getPost('id'));
        return $this->template->view('projects/materials/category_modal_form', $view_data);
    }
    ...
    function category_save() {
        ...
        if ($save_id) {
            echo json_encode(array("success" => true, "data" => $this->_categories_row_data($save_id), 'id' => $save_id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }

    function categories_data($project_id,$area_id) {
         validate_numeric_value($project_id);
        if ($project_id) {
            $view_data['project_id'] = $project_id;
            $view_data['area_id'] = $_REQUEST['id'];
            $view_data['project_info'] = $this->Projects_model->get_details(array("id" => $project_id))->getRow();
            $view_data["can_edit_invoices"] = $this->can_edit_invoices();
            return $this->template->view("projects/materials/categories", $view_data);
          }
    }

    //get data for items category list
    function categories_list_data($area_id = null) {
        if ($area_id === null) {
           echo $area_id = $this->request->getGet('area_id'); 
        }
        ///echo $area_id; die;
        $list_data = $this->Material_categories_model->get_categoriesByAreaId(array('area_id'=>$area_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_categories_make_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    //get an expnese category list row
    private function _categories_row_data($id) {
         $options = array("id" => $id);
        $data = $this->Material_categories_model->get_details($options)->getRow();
        return $this->_categories_make_row($data);
    }

    //prepare an item category list row
    private function _categories_make_row($data) {
    ...
    //  Materials add
     /*protected function validate_access_to_materials() {
        $access_invoice = $this->access_only_team_members("invoice");
        $access_estimate = $this->validate_access_to_materials("estimate");
   ...
   function material_list_data($project_id = "") {
    //$this->access_only_team_members();
    //$this->validate_access_to_materials();

    // Get the area_id and category_id from the POST request
    ...
    $list_data = $this->Materials_model->get_details($options)->getResult();

    $result = array();
	
    foreach ($list_data as $data) {
        $result[] = $this->_material_make_item_row($data);
    }
    ...
    echo json_encode(array("data" => $result));
	 ...
	 function getcategoryData() {
		...
		if ($area_id) {
			 $options = array("area_id" => $area_id);
			$categories = $this->Material_categories_model->get_specific_category($options)->getResultArray();
    ...
    private function _material_make_item_row($data) {
        $type = $data->unit_type ? $data->unit_type : "";
        ...
        if($this->can_edit_invoices()){ $actions=modal_anchor(get_uri("materials/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("materials/delete"), "data-action" => "delete")); }else{
                $actions='';
            }
        return array(
            modal_anchor(get_uri("materials/view"), $show_in_client_portal_icon . $data->title, array("title" => app_lang("item_details"), "data-post-id" => $data->id)),
    ...
    private function _get_categories_dropdown() {
        $categories = $this->Material_categories_model->get_all_where(array("deleted" => 0), 0, 0, "title")->getResult();
    ...
    private function _get_area_dropdown() {
        $categories = $this->Material_area_model->get_all_where(array("deleted" => 0), 0, 0, "areaname")->getResult();

        $categories_dropdown = array(array("id" => "", "text" => "- " . app_lang("area") . " -"));
        foreach ($categories as $category) {
            $categories_dropdown[] = array("id" => $category->id, "text" => $category->areaname);
        }
    ...
    function materials_list($project_id='') {
        $this->access_only_team_members();
        // $this->validate_access_to_materials();
        $view_data['project_id']=$project_id; //die;
        /*
        $view_data['categories_dropdown'] = $this->_get_categories_dropdown();
        $view_data['area_dropdown'] = $this->_get_area_dropdown();*/
        $view_data['area_data'] = $this->Material_area_model->get_details(array("project_id" => $project_id))->getResult();
            $view_data['category_data'] = $this->Material_categories_model->get_details(array("project_id" => $project_id))->getResult();
            $view_data["can_edit_invoices"] = $this->can_edit_invoices();
            
        return $this->template->rander("projects/materials/index", $view_data);
    }
    /*private function _get_categories_dropdown() {
        $categories = $this->Material_categories_model->get_all_where(array("deleted" => 0), 0, 0, "title")->getResult();
    ...
    private function _get_area_dropdown() {
        $areadata = $this->Material_area_model->get_details(array("deleted" => 0))->getResult();

        $area_dropdown = array(array("id" => "", "text" => "- " . app_lang("area") . " -"));
        foreach ($areadata as $area_data) {
            $area_dropdown[] = array("id" => $area_data->id, "text" => $area_data->areaname);
        }

        return json_encode($area_dropdown);
    }*/
    /* load expenses tab  */
    ...
    private function storeContractVersion($contract_id, $contract_data,$parent_contract_id='') {
                // Get the latest version information $this->login_user->is_admin
            if($parent_contract_id==''){
                $contract_data["version_number"] = '1.0';
                $contract_data["parent_contract_id"] = $contract_id;  
                $contract_data["created_by"] = $this->login_user->id;  
                
                $this->Contracts_model->ci_save($contract_data, $contract_id);
            }else{
                 $latestVersion = $this->Contracts_model->getLatestContractVersion($contract_id,$parent_contract_id);
                 $new_version_number = (float)$latestVersion->version_number + 0.1;        
                 $contract_data["version_number"] = (string)$new_version_number;
                 $contract_data["created_by"] = $this->login_user->id; 
                 $contract_data["parent_contract_id"] = $latestVersion->parent_contract_id ?: $contract_id;
                 $this->Contracts_model->ci_save($contract_data,$contract_id);
            }
        }
    ...
    private function _copy_related_items_to_contract($copy_items_from_estimate, $contract_id) {
    ...
    //  Project Assets 
      ...
      public function assets($project_id)
    {

        // print_r($this->login_user->is_admin); die;
        $options=array(
            'id'=>$project_id
        );
        $project = $this->Projects_model->get_details($options)->getResult(); 
        if (!$project) {
           // return redirect()->back()->with('error', 'Project not found.');
        }

        // Get active assignments for project (Projects_model method below)
        $data['project'] = $project;
        $data['project_id']=$project_id;
        $data['permissions']=$this->login_user->permissions;
        $data['is_admin']=$this->login_user->is_admin;
        $data['assignments'] = $this->Projects_model->get_project_assets($project_id);

        echo $this->template->view('projects/assets/assets_tab', $data);
    }

     public function returned_assets($project_id)
    {
        $options=array(
            'id'=>$project_id
        );
        ...
        $data['permissions']=$this->login_user->permissions;
        $data['is_admin']=$this->login_user->is_admin;
        $data['assignments'] = $this->Projects_model->get_project_returned_assets($project_id);

        echo $this->template->view('projects/assets/returned_assets_tab', $data);
    }
      ...
      public function get_billing_type()
            {
                ...
                if (!empty($asset_id)) {
                    $result = $this->Projects_model->get_billing_type($asset_id);
                    foreach ($result as $row) {
                        $html .= '<option value="' . $row->id . '">' . $row->asset_name . '</option>';
                    }
                } else {
                    $html .= '<option value="">Select assets first.</option>';
                }

                echo $html;
            }

    public function get_asset_billing_type()
            {
                $html = '';
                $asset_id = $this->request->getPost("asset_id");
                if (!empty($asset_id)) {
                    $result = $this->Projects_model->get_asset_billing_type($asset_id); //print_r($result); die;
                     $resResult= array(
                        'result'=>$result,
                        'response'=>'success',
                ...
                return $response=json_encode($resResult);
      ...
      public function get_assets_info()
            {
                ...
                $asset_id = $this->request->getPost("asset_id");
                if (!empty($asset_id)) {
                    $result = $this->Projects_model->get_assets($asset_id);
                    foreach ($result as $row) {
                        $html .= '<option value="' . $row->id . '">' . $row->asset_name . '</option>';
                    }
                } else {
    ...
    /**
     * Show Assign Asset form (modal / page)
     * URL: /projects/assign_asset_form/123
     */
    public function assign_asset_form($project_id)
    {
        $options=array(
            'id'=>$project_id
        );
        ...
        $data['project'] = $project;
        $data['assets']  = $this->Projects_model->get_assets_list();
        $data['units']   = $this->Projects_model->get_units_list(); 
        $data['billing_type_dropdown'] = $this->Projects_model->get_billing_type();
        echo $this->template->view('projects/assets/assign_asset_form', $data);
    }

    /**
     * POST handler to assign asset to project
     * URL: POST /projects/assign_asset
     */
    public function assign_asset()
    {
        ...
        $from_date   = (int) ($post['from_date'] ?? 0);
        $to_date   = (int) ($post['to_date'] ?? 0);
        $no_of_day   = (int) ($post['no_of_day'] ?? 0);
        $quantity   = (int) ($post['quantity'] ?? 0);
        $unit_id    = isset($post['unit_id']) ? (int)$post['unit_id'] : null;
        $remarks    = $post['remarks'] ?? null;
        $total_price    = $post['price'] ?? 0; 
        $unit_price    = $post['rate'] ?? 0; 
        $unit_quantity    = $post['unit_quantity'] ?? 0; 
        $billing_type    = $post['billing_type'] ?? 0; 

        $user_id    = function_exists('logged_user') ? logged_user()->id : (int)($post['assigned_by'] ?? 0);

        if ($quantity <= 0) {
            return redirect()->back()->with('error', 'Quantity must be at least 1.');
        }

        // Call Projects_model wrapper to do the assignment (transaction + ledger)
        $result = $this->projectsModel->assign_asset($project_id, $asset_id, $quantity, $unit_id,$total_price, $remarks, $user_id, $from_date , $to_date, $no_of_day,$unit_price,$unit_quantity, $billing_type);
       
        if ($result['success']) {
            return redirect()->to('/projects/view/' . $project_id)->with('success', $result['message']);
        } else {
    ...
    /**
     * Return assignment (mark returned)
     * URL: /projects/return_asset/45
     */
    public function return_asset($assignment_id)
    {
        $user_id = $this->login_user->id;
        $result = $this->Projects_model->return_asset($assignment_id, $user_id);

        if ($result['success']) {
            
            return redirect()->to('/projects/view/' . ($result['project_id'] ?? ''))->with('success', $result['message']);
        }
        return redirect()->back()->with('error', $result['message']);
    }

    /**
     * AJAX endpoint to get available qty for an asset
     * URL: POST /projects/ajax_asset_availability
     */
    public function ajax_asset_availability()
    {
        $asset_id = $this->request->getPost('asset_id'); 
        $asset = $this->Projects_model->get_assets($asset_id); 
        if (!$asset) {
            return $this->response->setJSON(['success'=>false,'available'=>0,'message'=>'Asset not found']);
        }
        return $this->response->setJSON(['success'=>true,'available'=>(int)$asset[0]->quantity]);
    }
    ...
    public function return_list(){
      ...
      // Return Asset
    public function return_asset_form()
    {
        $post = $this->input->post();
        $assignment_id = (int) ($post['assignment_id'] ?? 0);
        $return_quantity = (int) ($post['quantity'] ?? 0);
        $return_date = $post['return_date'] ?? date('Y-m-d');
        $status     = $post['status'] ?? 'returned';
        $condition  = $post['condition'] ?? 'good';
        $remarks    = $post['remarks'] ?? '';

        // Fetch assignment
        $assignment = $this->db->where('id', $assignment_id)->get('pcm_project_asset_assignments')->row();
        ...
        if (!$assignment->model_id) {
            $this->session->set_flashdata('error', 'Assigned asset has no model. Cannot return.');
            redirect($_SERVER['HTTP_REFERER']);
        }

        if ($return_quantity <= 0) {
            $this->session->set_flashdata('error', 'Return quantity must be greater than zero.');
            redirect($_SERVER['HTTP_REFERER']);
        }
        ...
        $this->db->insert('asset_returns', $data);

        $this->session->set_flashdata('success', 'Asset return submitted successfully.');
        redirect('/assets/return_list');
    }
