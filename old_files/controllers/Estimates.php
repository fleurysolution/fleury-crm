...
class Estimates extends Security_Controller {
    ...
    function item_modal_form() {
        ...
        if(!empty($view_data['model_info']->materials)){
        $view_data['materials_data']=$this->Estimate_items_model->get_material_detail($view_data['model_info']->materials);
        }
        
        $view_data['all_materials']=$this->Estimate_items_model->get_all_material();
        $view_data['material_categories']=$this->Estimate_items_model->get_materials_categories();
        $view_data['estimate_id'] = $estimate_id;
    ...
    function material_modal_form(){
        ...
        $view_data['model_info'] = $this->Estimate_materials_model->get_one($this->request->getPost('id'));
        if (!$estimate_id) {
            $estimate_id = $view_data['model_info']->estimate_id;
        }
        if(!empty($view_data['model_info']->materials)){
        $view_data['materials_data']=$this->Estimate_materials_model->get_material_detail($view_data['model_info']->materials);
        }
        
        $view_data['all_materials']=$this->Estimate_materials_model->get_all_material();
        $view_data['material_categories']=$this->Estimate_materials_model->get_materials_categories();
        $view_data['material_categories']=$this->Estimate_materials_model->get_materials_categories();
        $view_data['area_dropdown'] = $this->Estimate_materials_model->get_dropdown_list(array("areaname"));

        $view_data['estimate_id'] = $estimate_id;
        return $this->template->view('estimates/material_modal_form', $view_data);
    ...
    function save_item() {
        ...
        $estimate_item_data = array(
            ...
            "rate" => unformat_currency($this->request->getPost('estimate_item_rate')),
            "total" => $rate * $quantity,
            "material_categories" => $this->request->getPost('material_categories'),
    ...
    private function _make_item_row($data) {
        ...
        if ($data->description) {
         ...
         if (!empty($data->materials)) {
            //$allMaterials=explode(',',$data->materials); print_r($allMaterials);
            $dataMaterials=$this->Estimate_items_model->get_material_detail($data->materials);
            // print_r($dataMaterials); die;
            $item .='<div class="item-row">
                <table>';
                // <p  class="text-wrap">'.custom_nl2br($materialsData->description).'</p>
            foreach($dataMaterials as $materialsData){
            $item .='<tr><td><p>'.$materialsData->title.'</p>
             </td> <td> '.$materialsData->quantity.ucfirst($materialsData->unit_type).'</td><td>'. to_currency($materialsData->rate, $data->currency_symbol).' </td> </tr> ';
            }
            $item .='</table></div>';
    ...
    function get_estimate_materials() {
        $key = $this->request->getPost("category_id"); 
        $suggestion = array();
        $materialsData = $this->Estimate_items_model->get_materials_list($key);
    ...
    function save_scope(){
        ...
        $view_data['scope'] =$this->request->getPost('estimate_scope');
        $estimate_id = $this->Estimates_model->ci_save($view_data, $estimate_id);
        if($estimate_id){
         // echo json_encode(array("success" => true, "data" => $estimate_id, 'id' => $estimate_id, 'message' => app_lang('record_saved')));
        return json_encode(array("success" => true, "data" => $this->_row_data($estimate_id), 'id' => $estimate_id, 'message' => app_lang('record_saved')));
        } else {
