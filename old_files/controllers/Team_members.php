...
class Team_members extends Security_Controller {
    ...
    function __construct() {
        ...
        $this->Weekly_availability_model = model('App\Models\Weekly_availability_model');
        $this->Weekly_breaks_model       = model('App\Models\Weekly_breaks_model');
    }
    ...
    function modal_form() {
        ...
        $subdomain = explode('.', $parsedUrl)[0]; 
         ...
         $response = $client->post('https://bpms247.com/check-subscription-details', [
            'form_params' => [
                'subdomain' => $subdomain
            ]
        ]);
        ...
        $existclient = $this->Clients_model->get_clients_total_client($this->login_user->id);
      if ($result['max_no_of_staff'] == count($list_data)) {
             app_redirect_check('forbidden', 'You have already reached the maximum number of member allowed in your subscription.');
        }
    ...
    function add_team_member() {
        ...
        $user_data = array(
            ...
            "permanent_country" => $this->request->getPost('permanent_country'),            
            "permanent_zipcode" => $this->request->getPost('permanent_zipcode'),
    ...
    function list_data() {
        ...
        $list_data = $this->Users_model->get_details($options)->getResult();
        $result = array(); //print_r($list_data);
        foreach ($list_data as $data) {
            if($data->office_id){ $officeData=$this->Settings_model->get_name_data('offices',$data->office_id); $data->office_name=$officeData[0]->name; }else{ $data->office_name='N/A'; }
            if($data->region_id){ $regionData=$this->Settings_model->get_name_data('regions',$data->region_id); $data->region_name=$regionData[0]->name; }else{ $data->region_name='N/A'; }
            if($data->division_id){ $divisionData=$this->Settings_model->get_name_data('divisions',$data->division_id); $data->division_name=$divisionData[0]->name; }else{ $data->division_name='N/A'; }
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                $view_data['show_appointments_info ']=true;
    ...
    function save_general_info($user_id) {
        ...
        $user_data = array(
            ...
            "alternative_city" => $this->request->getPost('alternative_city'),
            "alternative_state" => $this->request->getPost('alternative_state'),
            "alternative_zipcode" => $this->request->getPost('alternative_zipcode'),
            "alternative_country" => $this->request->getPost('alternative_country'),
         ...
         if (strpos($this->request->getPost('ssn'), '*') === false) { 
            $user_data["ssn"] = $this->request->getPost('ssn');
        }
    ...
    //show yearly w2_form list of a team member
    function w2_form($user_id) {
        validate_numeric_value($user_id);
        $view_data["user_id"] = $user_id;
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("expenses", $this->login_user->is_admin, $this->login_user->user_type);
        return $this->template->view("team_members/w2_form_list", $view_data);
    }

    function add_w2_form($user_id) {
        validate_numeric_value($user_id);
        $this->Users_model->get_w2form_details();
        $view_data['title']='W2 Form';
        return $this->template->view("team_members/w2-form", $view_data);
    }

    //get the expnese list data
    function w2_list_data($recurring = false) {
        ...
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_w2_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    
      private function _make_w2_row($data) {
        return array(
            $data->id,
    ...
    private function _prepare_team_member_data($row_data) {
     ...
     public function get_offices_by_regionID($region_id) {
      $offices=$this->Settings_model->get_offices($region_id);
        echo json_encode($offices);
        }

    public function get_division_by_officeID($office_id) {
            $divisions = $this->Settings_model->get_divisions($office_id);
        ...
        public function team_availability($user_id)
            {
                validate_numeric_value($user_id);

                // Permission: admin OR owner OR allowed member-editor
                $this->update_only_allowed_members($user_id);

                // Fetch weekly availability + breaks
                $availability = $this->Weekly_availability_model->get_staff_weekly_availability((int)$user_id);
                $breaks       = $this->Weekly_breaks_model->get_staff_weekly_breaks((int)$user_id);
                ...
                // Render inside existing template system
                return $this->template->view("appointment_services/team_availability_form", $view_data);
            }       
            ...
            public function save_weekly_availability()
            {
                ...
                // Permission: admin OR owner OR allowed member-editor
                $this->update_only_allowed_members($staff_id);

                $day       = (string)$this->request->getPost("day_of_week");
                $start     = (string)$this->request->getPost("start_time");
                $end       = (string)$this->request->getPost("end_time");
                $isAvail   = (int)$this->request->getPost("is_available");

                $allowedDays = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
                if (!in_array($day, $allowedDays, true)) {
                    return $this->response->setJSON(["success" => false, "message" => "Invalid day."]);
                }

                if (!$this->Weekly_availability_model->is_valid_time_range($start, $end)) {
                    return $this->response->setJSON(["success" => false, "message" => "Invalid time range."]);
                }

                $ok = $this->Weekly_availability_model->upsert_staff_day_window(
                    $staff_id, $day, $start, $end, $isAvail
                );

                return $this->response->setJSON([
                    "success" => (bool)$ok,
                    "message" => $ok ? "Availability saved." : "Unable to save availability."
                ]);
            }
        public function save_weekly_break()
        {
            $this->validate_submitted_data([
                "staff_id"    => "required|numeric",
                "day_of_week" => "required",
            ...
            $start = (string)$this->request->getPost("start_time");
            $end   = (string)$this->request->getPost("end_time");
            $title = trim((string)$this->request->getPost("title")) ?: "Break";
            ...
            if (!$this->Weekly_breaks_model->is_valid_time_range($start, $end)) {
                return $this->response->setJSON(["success" => false, "message" => "Invalid break time range."]);
            }

            // Must be inside availability window IF that day is available
            $win = $this->Weekly_availability_model->get_staff_day_window($staff_id, $day);
            if ($win && (int)$win->is_available === 1) {
                if (!$this->Weekly_breaks_model->is_within_window($start, $end, $win->start_time, $win->end_time)) {
                    return $this->response->setJSON(["success" => false, "message" => "Break must be inside availability window."]);
                }
            }

            // No overlap with existing breaks
            if ($this->Weekly_breaks_model->break_overlaps_existing($staff_id, $day, $start, $end)) {
                return $this->response->setJSON(["success" => false, "message" => "Break overlaps an existing break."]);
            }

            $id = $this->Weekly_breaks_model->add_break($staff_id, $day, $start, $end, $title);

            return $this->response->setJSON([
                "success" => (bool)$id,
                "message" => $id ? "Break added." : "Unable to save break."
            ]);
        }

        public function delete_weekly_break()
        {
            $this->validate_submitted_data([
                "id" => "required|numeric"
            ], true);
            ...
            // Optional: fetch break to validate permission by staff_id
            $break = $this->Weekly_breaks_model->get_one($id);
            if ($break && isset($break->staff_id)) {
                $this->update_only_allowed_members((int)$break->staff_id);
            } else {
                return $this->response->setJSON(["success" => false, "message" => "Break not found."]);
            }

            $ok = $this->Weekly_breaks_model->soft_delete_break($id);

            return $this->response->setJSON([
                "success" => (bool)$ok,
                "message" => $ok ? "Break deleted." : "Unable to delete break."
            ]);
        }
