...
class Materials extends Security_Controller {
    ...
    function materials_list($category_id='') {
        $this->access_only_team_members();
        $this->validate_access_to_materials();
        $view_data['category_id']=$category_id; //die;
        $view_data['categories_dropdown'] = $this->_get_categories_dropdown();
    ...
    function list_data($category_id="") {
        $this->access_only_team_members();
        $this->validate_access_to_materials();

       //echo $category_id = $this->request->getPost('category_id');
       // echo  $category_id;
    ...
    function view() {
        ...
        $model_info = $this->Materials_model->get_details(array("id" => $this->request->getPost('id'), "login_user_id" => $this->login_user->id))->getRow();

        $view_data['model_info'] = $model_info;
        $view_data["client_info"] = $this->Clients_model->get_one($this->login_user->client_id);

        return $this->template->view('materials/view', $view_data);
    }
    ...
    function save_files_sort() {
        ...
        if ($id && $sort_values) {
            ...
            if ($item_info->id) {
                ...
                $this->Materials_model->ci_save($item_data, $id);
            }
        }
    }

    function import_materials_modal_form() {
        $this->access_only_team_members();
        $this->validate_access_to_materials();

        return $this->template->view("materials/import_materials_modal_form");
    }
    ...
    function save_item_from_excel_file() {
        ...
        if (!$this->validate_import_materials_file_data(true)) {
            echo json_encode(array('success' => false, 'message' => app_lang('error_occurred')));
        }
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            $item_data_array = $this->_prepare_material_data($value, $allowed_headers);
            $item_data = get_array_value($item_data_array, "material_data");
            ...
            //save item data
            $item_save_id = $this->Materials_model->ci_save($item_data);
            if (!$item_save_id) {
                continue;
            }
        }
    ...
    private function _get_material_category_id($category = "") {
        if (!$category) {
            return false;
        }

        $existing_category = $this->Material_categories_model->get_one_where(array("title" => $category, "deleted" => 0));
    ...
    function validate_import_materials_file_data($check_on_submit = false) {
        $this->access_only_team_members();
        $this->validate_access_to_materials();
    ...
    private function _prepare_material_data($data_row, $allowed_headers) {
        //prepare item data
        $item_data = array();
        ...
        foreach ($data_row as $row_data_key => $row_data_value) { //row values
            ...
            if ($header_key_value == "category") { //we've to make category data differently
                $item_data["category_id"] = $this->_get_material_category_id($row_data_value);
