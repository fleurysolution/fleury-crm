...
class Contracts extends Security_Controller {
    ...
    // Ammend Contract Model
    function ammend_model_form(){
        $this->access_only_allowed_members();
    ...
    function save() {
        ...
        $parent_contract_id=$this->request->getPost('parent_contract_id');
        ...
        if ($contract_id) {
            // version store 
           $this->storeContractVersion($contract_id, $contract_data,$parent_contract_id); echo $id;
            if(!empty($parent_contract_id)){
                ...
                $contract_items = $this->Contract_items_model->get_all_where(array("contract_id" => $parent_contract_id, "deleted" => 0))->getResult();
    ...
    // Contract version 
    // Versioning function to store contract versions
        /*private function storeContractVersion($contract_id, $contract_data,$id) {
           // echo $contract_id;
            
            $latestVersion = $this->Contracts_model->getLatestContractVersion($contract_id);
           // print_r($latestVersion); die;
            if (!$latestVersion) {
                //echo "I am not the latest version";
                ...
                //echo "I am latest version";
                //print_r($latestVersion);
                //$parent_contract_id= $this->Contracts_model->get_parent_contract_id($latestVersion->parent_contract_id); print_r($parent_contract_id);
                $new_version_number = (float)$latestVersion->version_number + 0.1;
                $contract_data["version_number"] = (string)$new_version_number;
                $contract_data["parent_contract_id"] = $latestVersion->parent_contract_id;
                $this->Contracts_model->ci_save($contract_data);
            }
        }*/
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if($data->parent_contract_id){
            $parent_contract_url = anchor(get_uri("contracts/view/" . $data->parent_contract_id), 'Contracts #'.$data->parent_contract_id);
            
        }else{
            $parent_contract_url = 'N/A';
        }

        $contract_url = "";
        if ($this->login_user->user_type == "staff") {
            $title=$data->title. ' ('.$data->version_number.')';
            $contract_url = anchor(get_uri("contracts/view/" . $data->id), $title);
        } else {
            //for client
            $title=$data->title. ' ('.$data->version_number.')';
            $contract_url = anchor(get_uri("contracts/preview/" . $data->id), $title);
        }
        /*
        $contract_url = "";
        if ($this->login_user->user_type == "staff") {
        ...
        $row_data = array(
            ...
            to_currency($contract_value, $data->currency_symbol),
    ...
    function view($contract_id = 0) {
        ...
        if ($contract_id) {
            ...
            if ($view_data) {
                ...
                $view_data['all_versions']=$this->Contracts_model->getAllContractVersions($contract_id)->getResult();
