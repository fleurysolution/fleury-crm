...
class Webhooks_listener extends App_Controller {
    ...
    function stripe_subscription($key) {
       //echo $key; 
        ...
        //print_r($payloads); echo $payloads->type; die;
        if ($payloads->type === "invoice.payment_succeeded") {
            $this->subscription_payment_succeeded($payloads);
        }

         if ($payloads->type === "customer.subscription.created") {
            $this->subscription_payment_succeeded($payloads);
        }

         if ($payloads->type === "customer.subscription.updated") {
            $this->subscription_payment_succeeded($payloads);
        }

        if ($payloads->type === "customer.subscription.deleted") {
            $this->subscription_payment_succeeded($payloads);
        }
