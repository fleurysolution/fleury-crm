...
class Invoices extends Security_Controller {
    ...
    function index($tab = "", $status = "", $selected_currency = "") {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            //print_r($this->can_client_access('invoice')); die;
            if ($this->can_client_access("invoice") != 1) {
                app_redirect("forbidden");
            }
            // $view_data['project_dropdown']='';
            $view_data['project_dropdown']=$this->get_project_suggestion($this->login_user->client_id);
     ...
     function active_invoices($tab = "", $status = "", $selected_currency = "") {
        $this->check_module_availability("module_invoice");
    ...
    function invoice_list_data_of_client($client_id) {
        ...
        $list_data = $this->Invoices_model->get_invoice_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row($data, $custom_fields);
        }
    ...
    function active_invoice_list_data_of_client($client_id) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
        ...
        $list_data = $this->Invoices_model->get_active_invoice_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_client_row($data, $custom_fields);
        }
        echo json_encode(array("data" => $result));
    }


    function cancelled_invoice_list_data_of_client($client_id) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
        ...
        $list_data = $this->Invoices_model->get_cancelled_invoice_details($options)->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_client_row($data, $custom_fields);
        }
    ...
    function invoice_list_data_of_project($project_id, $client_id = 0) {
     ...
     function active_invoice_list_data_of_project($project_id, $client_id = 0) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
     ...
     function cancelled_invoice_list_data_of_project($project_id, $client_id = 0) {
        if (!$this->can_view_invoices($client_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $credit_note_url = "";
            ...
            if(!empty($data->payment_received)){ $payment_received=to_currency($data->payment_received, $data->currency_symbol); }else{ $payment_received="0"; }
                if(!empty($data->project_price)){ $project_price=to_currency($data->project_price, $data->currency_symbol); }else{ $project_price="0"; }
                if(!empty($invoice_value)){ $invoice_value=$invoice_value; }else{ $invoice_value="0"; }
            ...
            $row_data = array(
                ...
                if(empty($data->company_name)){
                    $anchor='Company Name Not Available';
                }else{
                    $anchor= anchor(get_uri("clients/view/" . $data->client_id), $data->company_name);                    
                }
                $row_data = array(
                    $data->id,
                    $invoice_url,
     ...
     private function _make_client_row($data, $custom_fields) {
        $invoice_url = "";
        $main_invoice_url = "";
        $credit_note_url = "";
            ...
            $row_data = array(
                ...
                $data->due_date,
                $due_date,
                // to_currency($data->project_price, $data->currency_symbol),
                to_currency($invoice_value, $data->currency_symbol),
    ...
    function project_download_pdf($invoice_id = 0, $mode = "download", $user_language = "") {
        if ($invoice_id) {
            validate_numeric_value($invoice_id);
            $invoice_data = get_project_invoice_making_data($invoice_id,$this->login_user->client_id);
            $this->_check_invoice_access_permission($invoice_data);
            ...
            if ($user_language) {
                ...
                project_prepare_invoice_pdf($invoice_data, $mode);
    ...
    // Voide Note and Update status as void invoice
   
    function void_note_form($invoice_id) {
        if (!$this->can_edit_invoices()) {
            app_redirect("forbidden");
        }
        ...
        return $this->template->view('invoices/invoice_void_note', $view_data);
    }

    function void_invoice_status() {
        if (!$this->can_edit_invoices()) {
            app_redirect("forbidden");
        }
        ...
        //void note
        if ($invoice_id) {
            //save void status for main invoice
            $invoice_data = array("status" => "void");
            $this->Invoices_model->ci_save($void_note_data, $invoice_id);
            echo json_encode(array("success" => true, 'id' => $invoice_id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    }
    ...
    //load invoice payments section
    function pending_payments($invoice_id) {
        if (!$this->can_view_invoices()) {
            app_redirect("forbidden");
        }
        ...
        if ($invoice_id) {
            ...
            return $this->template->view("invoices/payments/pending-payments", $view_data);
        } else {
            show_404();
        }
    }
    ...
    private function _make_summary_row($data, $default_currency_symbol) {
        ...
        if($data->client_name==''){
            $data->client_name='Client not available.';
        }
        $row_data = array(
