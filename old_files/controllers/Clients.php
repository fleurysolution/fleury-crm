...
class Clients extends Security_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if(!empty($data->subdomain)){ $subdomain_url="https://".$data->subdomain.".bpms247.com"; }else{ $subdomain_url="N/A"; }
        $row_data = array(
            $data->id,
            ...
            '<a href="'.$subdomain_url.'" target="_blank">'.$subdomain_url.' </a> ',
        );
    ...
    function __project_dropdown($client_id){
        $options=array('client_id'=>$client_id);
        $projectData=$this->Projects_model->get_details($options)->getResult;
    }
    ...
    function invoices($client_id) {
        ...
        if ($client_id) {
            ...
            $client_options=array('client_id'=>$client_id);
            $project_data=$this->Projects_model->get_details($client_options)->getResult();
             
            $project_suggestions[]=array("id" => "", "text" => "- " . app_lang('projects') . " -");
            foreach($project_data as $projects){
                $project_suggestions[]=array("id" => $projects->id, "text" => $projects->title);
            }
             $view_data['project_dropdown'] = json_encode($project_suggestions);
    ...
    function my_subscription($contact_id){
        $this->access_only_allowed_members_or_contact_personally($contact_id);
        $view_data['user_info'] = $this->Users_model->get_one($contact_id);
        $this->_validate_client_view_access($view_data['user_info']->client_id);
        $view_data['client_info'] = $this->Clients_model->get_one($view_data['user_info']->client_id);
        $view_data['can_edit_clients'] = $this->can_edit_clients(); 
        $view_data['subscription_data']=$this->Subscriptions_model->get_details(array('client_id'=>$view_data['user_info']->client_id))->getResult();
        if(!empty($view_data['subscription_data'])){
         //echo $view_data['subscription_data'][0]->id; die;
        $view_data['subscription_item_data']=$this->Subscription_items_model->get_details(array('subscription_id'=>$view_data['subscription_data'][0]->id))->getResult();
    }else{
        $view_data['subscription_data']=array();
         $view_data['subscription_item_data']=array();
    }

        return $this->template->view("users/my-subscriptions", $view_data);
    }

    function upgrade_subscription(){
        $list_data = $this->Package_price_model->get_details()->getResult();
        $view_data["priceList"] =$list_data;
       //  $this->template->view("users/upgrade-subscription", $view_data);
      return  $this->template->rander("users/upgrade-subscription", $view_data);
    }

    function cancel_subscription_request(){
        $list_data = $this->Package_price_model->get_details()->getResult();
        $view_data["priceList"] =$list_data;
        return $this->template->rander("users/cancel-subscription-request", $view_data);
    }
