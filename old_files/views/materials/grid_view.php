...
<div id="page-content" class="page-wrapper clearfix <?php echo $has_banner_image ? "pt0" : ""; ?>">
    <div class="<?php echo isset($login_user->id) ? "" : "container store-page"; ?>">
        ...
        <div class="row" id="materials-container">
            <?php echo view("materials/materials_grid_data"); ?>
        </div>
    </div>
</div>
    ...
    $(document).ready(function () {
        ...
        $searchBox.on("keyup", function (e) {
            if (!(e.which >= 37 && e.which <= 40)) {
                ...
                var wait = setTimeout(getmaterialsuggestions, 200);
        ...
        function getmaterialsuggestions() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        $("#materials-container").html(response.data);
                    }
                }
            });
        }
        ...
        $("body").on("click", ".item-add-to-cart-btn", function () {
            ...
            //add item to the order materials table and show count on cart box
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        //reload cart value
                        window.countCartmaterials();

                        //change the selector when it's added from view modal
                        if ($("#ajaxModal").hasClass('in')) {
                            $instance = $("#materials-container").find("[data-item_id='" + itemId + "']");
