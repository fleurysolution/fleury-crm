                        ...
                        <div class="title-button-group mr0">
                            <span class="dropdown inline-block mt15">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    ...
                                    $estimateTotal=$estimate_total_summary->estimate_total; ?>
                                    <?php if ($client_info->address) { 
                                     $address= nl2br($client_info->address); 
                                     }
                                     if ($client_info->city) { 
                                      $address.= ', '.$client_info->city; 
                                    } 
                                    if ($client_info->state) { 
                                        $address.= ', '.$client_info->state;
                                    } 
                                    if ($client_info->zip) { $address.= ', '.$client_info->zip; 
                                    } 
                                    if ($client_info->country) { 
                                          $address.= ', '.$client_info->country; 
                                     } 
        ?>
                                        <li role="presentation" class="dropdown-divider"></li>
                                        <?php if ($can_create_projects && !$estimate_info->project_id) { ?>
                                            <li role="presentation"><?php echo modal_anchor(get_uri("projects/modal_form"), "<i data-feather='command' class='icon-16'></i> " . app_lang('create_project'), array("data-post-context" => "estimate", "data-post-context_id" => $estimate_info->id, "data-post-estimate_title" => $estimate_info->estimate_title,"data-post-estimate_scope" => base64_encode($estimate_info->scope),"data-post-estimate_total" => base64_encode($estimateTotal),"data-post-estimate_location" => base64_encode($address), "title" => app_lang('create_project'), "data-post-client_id" => $estimate_info->client_id, "class" => "dropdown-item")); ?> </li>
                                        <?php } ?>
                    ...
                    <ul id="estimate-tabs" data-bs-toggle="ajax-tab" class="nav nav-pills rounded classic mb20 scrollable-tabs border-white" role="tablist">
                        ...
                        <li><a role="presentation" data-bs-toggle="tab"  href="javascript:;" data-bs-target="#estimate-scope-section"><?php echo app_lang("scope"); ?></a></li>
                ...
                <div class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane fade" id="estimate-scope-section">
                        <?php echo view("estimates/scope"); ?>
                    </div>
