...
<?php  $scope_data=$estimate_info->scope;
 $itemData='<table class="table-responsive" style="width: 100%;">
    <tr style="font-weight: bold; background-color: '.$color.'; color: #fff;  ">
        <th style="width: 45%; border-right: 1px solid #eee;"> '. app_lang("item").' </th>
        <th style="text-align: center;  width: 15%; border-right: 1px solid #eee;">  '.app_lang("quantity").'</th>
        <th style="text-align: right;  width: 20%; border-right: 1px solid #eee;"> '.app_lang("rate").'</th>
        <th style="text-align: right;  width: 20%; "> '.app_lang("total").'</th>
    </tr>';
    
    foreach ($estimate_items as $item) {
    
       $itemData .='<tr style="background-color: #f4f4f4; ">
            <td style="width: 45%; border: 1px solid #fff; padding: 10px; hyphens: auto;">'.$item->title.'
                <br />
                <span style="color: #888; font-size: 90%;">'. custom_nl2br($item->description ? process_images_from_content($item->description) : "").'</span>
            </td>
            <td style="text-align: center; width: 15%; border: 1px solid #fff;"> '.$item->quantity . ' ' . $item->unit_type.'</td>
            <td style="text-align: right; width: 20%; border: 1px solid #fff;"> '.to_currency($item->rate, $item->currency_symbol).'</td>
            <td style="text-align: right; width: 20%; border: 1px solid #fff;">'.to_currency($item->total, $item->currency_symbol).'</td>
        </tr>';
 } 
    $itemData .='<tr>
        <td colspan="3" style="text-align: right;">'.app_lang("sub_total").'</td>
        <td style="text-align: right; width: 20%; border: 1px solid #fff; background-color: #f4f4f4;">
            '. to_currency($estimate_total_summary->estimate_subtotal, $estimate_total_summary->currency_symbol).'
        </td>
    </tr>';
    if ($estimate_total_summary->discount_total && $estimate_total_summary->discount_type == "before_tax") {
     ...
     if ($estimate_total_summary->tax) { 
        $itemData.='<tr>
            <td colspan="3" style="text-align: right;">'.$estimate_total_summary->tax_name.'</td>
            <td style="text-align: right; width: 20%; border: 1px solid #fff; background-color: #f4f4f4;">
                '.to_currency($estimate_total_summary->tax, $estimate_total_summary->currency_symbol).'
            </td>
        </tr>';
     } 
     if ($estimate_total_summary->tax2) {
       $itemData.='<tr>
            <td colspan="3" style="text-align: right;">'.$estimate_total_summary->tax_name2.'</td>
            <td style="text-align: right; width: 20%; border: 1px solid #fff; background-color: #f4f4f4;">
                '.to_currency($estimate_total_summary->tax2, $estimate_total_summary->currency_symbol).'
            </td>
        </tr>';
    } 
    ...
    $itemData.='<tr>
        <td colspan="3" style="text-align: right;">'.app_lang("total").'</td>
        <td style="text-align: right; width: 20%; background-color: '.$color.'; color: #fff;">
            '.to_currency($estimate_total_summary->estimate_total, $estimate_total_summary->currency_symbol).'
        </td>
    </tr>
</table>';
$scope_data=str_replace('{ITEMS}',$itemData,$scope_data);
echo $scope_data;
?> 
