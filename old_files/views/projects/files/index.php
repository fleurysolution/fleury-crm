    ...
    <ul id="project-files-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title" role="tablist">
        ...
        <div class="tab-title clearfix no-border">
            <div class="title-button-group">
                ...
                <?php echo anchor("", " " . app_lang("share_now_to_client"), array("title" => app_lang("share_now_to_client"), "id" => "share-now-multiple-file-btn", "class" => "btn btn-default hide")); ?>
                ...
                if ($can_add_files) {
                    ...
                    echo modal_anchor(get_uri("projects/capture_image"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('capture_picture'), array("class" => "btn btn-default", "title" => app_lang('add_files'), "data-post-project_id" => $project_id, "id" => "file_or_category_add_button"));
                     /*echo modal_anchor(get_uri("projects/print_report"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('capture_picture'), array("class" => "btn btn-default", "title" => app_lang('add_files'), "data-post-project_id" => $project_id, "id" => "file_or_category_add_button"));*/
                }
            ...
            <div class="card border-top-0 rounded-top-0">
                <div class="table-responsive">
                   ...
                   <table id="project-file-table" class="display" width="100%">
                    ...
                    <select name="layout_type" class="form-select d-inline-block" style="width: auto; display:inline-block; margin-left:10px;">
                        <option value="1">Print 1 per Page</option>
                        <option value="2" selected>Print 2 per Page</option>
                        <option value="3">Print 3 per Page</option>
                    </select>
...
<div class="modal fade" id="imagePreviewModalList" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content bg-transparent border-0 shadow-none">
      <div class="modal-body text-center position-relative p-0">
        ...
        <img id="previewImageList" src="" alt="File Preview" class="img-fluid rounded" style="max-height:90vh;">
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('body').on('click', '[data-act=download-multiple-file-checkbox]', function() {
            ...
            $("#share-now-multiple-file-btn").attr("href", "<?php echo_uri("projects/share_now_multiple_files/"); ?>" + serializeOfArray);

            if (fields.length) {
                $("#download-multiple-file-btn").removeClass("hide");
                $("#delete-multiple-file-btn").removeClass("hide");
                $("#share-now-multiple-file-btn").removeClass("hide");
        ...
        //trigger share operation for multiple delete
        $("#share-now-multiple-file-btn").click(function() {
            $(this).addClass("hide");
            $("#select-un-select-all-file-btn").addClass("hide");
            $("#download-multiple-file-btn").addClass("hide");
            $("#delete-multiple-file-btn").addClass("hide");
...
<script>
$(document).on("click", ".open-image-modalList", function() {
    var imageUrl = $(this).data("image-url");
    $("#previewImageList").attr("src", imageUrl);
    $("#imagePreviewModalList").modal("show");
});
</script>
