...
<?php
//echo $project_info->status; print_r($project_info);
if($project_info->status_key_name =='completed'){
// if ($can_edit_timesheet_settings || $can_edit_slack_settings || $can_create_projects) {
    echo anchor(get_uri("projects/project_invoice_preview/$project_info->id"), "<i data-feather='download' class='icon-16 mr5'></i> " . app_lang('download_project_invoice_pdf'), array("class" => "btn btn-default", "title" => app_lang('download_project_invoice_pdf'), "data-post-project_id" => $project_info->id));
// }
}
?>