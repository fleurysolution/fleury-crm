<script>
     function disable_payment(clientpay){                
        var invoice_due = <?php echo $amount; ?>;
               // Validate clientpay to allow only numbers and a single decimal point
            clientpay = clientpay.replace(/[^0-9.]/g, ''); // Remove all but numbers and decimal
            if ((clientpay.match(/\./g) || []).length > 1) { // Allow only one decimal
                clientpay = clientpay.slice(0, -1);
            }

            // Update the input field with the validated value
            document.getElementById('invoice_payment_amount').value = clientpay;

            // Check if clientpay is within the allowed range
            if (parseFloat(clientpay) <= invoice_due) {
                document.getElementById('submitbuttnpayment').style.display = 'block'; // Show the submit button if valid
                document.getElementById('error_message').style.display = 'none'; // Hide error message if valid
                return true;
            } else {           
                document.getElementById('invoice_payment_amount').value = '';
                document.getElementById('submitbuttnpayment').style.display = 'none'; // Hide the submit button if invalid
                document.getElementById('error_message').style.display = 'block'; // Show error message if invalid
                return false;
            }
    }
            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    echo form_input(array(
                        ...
                        "data-rule-required" => true,
                        "onchange"=>'disable_payment(this.value);',
                        "data-msg-required" => app_lang("field_required"),
                    ));
                    ?>
                    <span id="error_message" style="color: red; display: none;">Amount exceeds invoice due amount. It do not allow to enter charecters or special charecters.</span>
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary" id="submitbuttnpayment"><span data-feather="check-circle" class="icon-16"></span> <?php echo app_lang('save'); ?></button>
</div>
<?php echo form_close(); ?>
