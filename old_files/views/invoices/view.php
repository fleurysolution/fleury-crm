                ...
                <div class="invoice-title-section">
                    <div class="page-title no-bg clearfix mb5 no-border">
                        ...
                        <div class="title-button-group mr0">
                            <!-- Start -->
                           <!--  <?php echo ajax_anchor(get_uri("invoices/update_invoice_status/" . $invoice_info->id . "/void"), "<i data-feather='check' class='icon-16'></i> " . app_lang('void'), array("data-reload-on-success" => "1", "class" => "btn-info text-white ")); ?>
                          -->  <!--  <a href="#" class="btn btn-default" title="Add payment" data-post-invoice_id="41" data-act="ajax-modal" data-title="Add payment" data-action-url="https://pcm.fleurysolutions.com/invoice_payments/payment_modal_form"> Add payment</a> -->
<!-- End -->
                            <?php if ($invoice_status !== "cancelled" && $invoice_info->status !== "credited" && $can_edit_invoices) { 
                                
                                if($invoice_info->invoice_total > $invoice_info->payment_received){
                                ?>
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    ...
                                    <?php if ($can_edit_invoices && $invoice_info->type == "invoice") { ?>
                                        ...
                                        <?php } else if ($invoice_status == "not_paid" || $invoice_status == "overdue" || $invoice_status == "partially_paid") { ?>
                                            ...
                                            <li role="presentation"><?php echo modal_anchor(get_uri("invoices/void_note_form/" . $invoice_info->id), "<i data-feather='file-minus' class='icon-16'></i> " . app_lang('mark_invoice_as_void'), array("title" => app_lang("mark_invoice_as_void"), "data-post-id" => $invoice_info->id, "class" => "dropdown-item")); ?> </li>
                                        <?php } ?>
...
<!-- 
                                        <li role="presentation"><?php //echo ajax_anchor(get_uri("invoices/update_invoice_status/" . $invoice_info->id . "/void"), "<i data-feather='check' class='icon-16'></i> " . app_lang('mark_invoice_as_void'),"data-action" => "delete-confirmation", array("data-reload-on-success" => "1", "class" => "dropdown-item")); ?> </li> , "onclick"=>"return confirm('Are you sure?')"-->
                    ...
                    <ul id="invoice-tabs" data-bs-toggle="ajax-tab" class="nav nav-pills rounded classic mb20 scrollable-tabs border-white" role="tablist">
                        ...
                        <?php if ($invoice_info->type == "invoice") { ?>
                            ...
                            <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("invoices/pending_payments/" . $invoice_info->id); ?>" data-bs-target="#invoice-pending-payments-section"><?php echo app_lang('pending_payments'); ?></a></li>
                ...
                <div class="tab-content">
                    ...
                    <?php if ($invoice_info->type == "invoice") { ?>
                        ...
                        <div role="tabpanel" class="tab-pane fade grid-button" id="invoice-pending-payments-section"></div>
                        <?php if ($invoice_info->recurring) { ?>
